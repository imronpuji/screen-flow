/**
 * After ffmpeg writes temp export.mp4, offer Save As (Documents/Screen Flow)
...
/** Best-effort: remove the MP4 and empty session dir under temp/screen-flow. */
...
/**
 * Show Save As (or use destinationPath for tests) and copy the temp MP4 out.
 */
export async function saveExportedMp4(
  ...
  if (!stat.isFile() || stat.size === 0) {
    throw new Error('Export MP4 is empty or not a file')
  }
  ...
  if (request.destinationPath != null && request.destinationPath.trim()) {
    ...
    destination = assertSafeSaveDestination(request.destinationPath)
  } else {
    ...
    const defaultPath = resolveDocumentsDefaultPath(
      ...
      request.defaultFileName
        ? sanitizeExportBasename(request.defaultFileName)
        : defaultExportBasename(),
    )
    ...
    destination = assertSafeSaveDestination(result.filePath)
  }
  ...
  if (!outStat.isFile() || outStat.size === 0) {
    throw new Error('Failed to write saved MP4')
  }
